<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@ page import="java.sql.*,jxl.*,java.io.*,java.util.*,Table.Road_data" %>
<%@ page import="query.SQLServerInfo,query.FileUploadInfo" %>
<%
	request.setCharacterEncoding("utf-8");
	
	ArrayList<Road_data> arrRow = new ArrayList<Road_data>();
	try {		
		Workbook wb = null;
		Cell cell = null;
		InputStream stream = new FileInputStream(FileUploadInfo.getFileUploadDirectory() + "Road_data.xls");
		...
		for (int i = 1; i < sheet.getRows(); ++i) {
			...
			Road_data row = new Road_data(str[0], str[1], str[2], str[3], str[4]);
			if (row.checkRow()) {
				arrRow.add(row);
			}
		}
...
<body>
	<%
		...
		try {
			...
			String sql = "insert into Road_data values (?, ?, ?, ?, ?)";
			PreparedStatement ps = conn.prepareStatement(sql);
			int batchSize = Integer.parseInt(request.getParameter("batchsize"));
			int count = 0;
			for (Road_data row : arrRow){
				ps.setString(1, row.getKeyNum());
			    ps.setInt(2, row.getCellID());
			    ps.setFloat(3, row.getLatitude());
			    ps.setFloat(4, row.getLongitude());
			    ps.setFloat(5, row.getRxLev());
			    ps.addBatch();
